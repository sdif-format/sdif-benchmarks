...
Usage:
    uv run sdif-benchmarks [--skip TRACK] [--only TRACK]
    SDIF_BENCHMARK_RETRIEVAL=1 ANTHROPIC_API_KEY=<key> uv run sdif-benchmarks
...
def _extract_semantic_scorecard(result_dir: Path) -> dict[str, Any]:
    summary = json.loads((result_dir / "summary.json").read_text())
    values: list[float] = []
    for doc in summary.get("documents", []):
        for fmt in doc.get("formats", []):
            if fmt.get("format") != "SDIF":
                continue
            for key in (
                "relationStructuralFidelity",
                "ruleStructuralFidelity",
                "tableStructuralFidelity",
                "fieldStructuralFidelity",
            ):
                value = fmt.get(key)
    ...
    return {
        "metric": "sdifSemanticAxisFidelity",
        "value": round(avg * 100, 1) if avg is not None else None,
        "unit": "%",
        "description": (
            f"SDIF structural semantic-axis recovery: {avg * 100:.1f}%"
            if avg is not None
            else "n/a"
        ),
    }
...
def _extract_operability_scorecard(result_dir: Path) -> dict[str, Any]:
    summary = json.loads((result_dir / "summary.json").read_text())
    ...
    enabled = sum(
        1
        for key in (
            "standardCanonicalForm",
            "builtinCanonicalForm",
            "stableHash",
            "schemaValidation",
            "nativeRelationSupport",
            "ruleDeclarationSupport",
            "semanticTypeVocabulary",
            "deterministicOutput",
        )
...
_SCORECARD_EXTRACTORS = {
    ...
    "semantic_fidelity": _extract_semantic_scorecard,
    "operability": _extract_operability_scorecard,
...
def _run_track(track: dict[str, Any], *, retrieval_enabled: bool, env: dict[str, str]) -> bool:
    cmd = [sys.executable, "-m", f"sdif_benchmarks.tracks.{track['id']}"]
    track_env = {**os.environ, **env}
    if track["id"] == "retrieval_accuracy":
        if not retrieval_enabled:
            print("  ⏭  Skipping retrieval_accuracy (set SDIF_BENCHMARK_RETRIEVAL=1 to enable)")
            return False
        track_env["SDIF_BENCHMARK_RETRIEVAL"] = "1"

    print(f"\n{'=' * 60}")
    print(f"  Running: {track['label']} (sdif_benchmarks.tracks.{track['id']})")
    print(f"{'=' * 60}")
    t0 = time.monotonic()
...
def main() -> None:
    ...
    try:
        ...
        index_sdif = "# BenchmarkSuiteReport (SDIF rendering skipped — see index.json)\n"
